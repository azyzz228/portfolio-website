...
function HackathonCard() {
    ...
    return (
        <div className=" flex flex-row items-center justify-start flex-wrap gap-16 p-6 sm:p-16 font-mulish">


            {hackathons.map((item, key) => (
                <div className='w-[384px] bg-white shadow-xl rounded-2xl p-12 relative overflow-hidden font-mulish transform ease-in-out duration-200  hover:-translate-y-1 group' key={key}>

                    <img src={item.image} alt="" className='transform opacity-50 rounded-xl absolute -top-3 -right-3 group-hover:opacity-70 ' />
